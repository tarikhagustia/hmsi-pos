...
@section('content')
    <x-main-content title="Master Data">
        <div class="card">
            <form action="{{ request()->isEditing ?  route('users.update', ['user' => $user->id]) : route('users.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                {{ request()->isEditing ? method_field('PUT') : '' }}
                <div class="card-header">
                    <h4>{{ request()->isEditing ? 'Sunting Pengguna' : 'Tambah Pengguna' }}</h4>
                </div>
                <div class="card-body">
                    ...
                    <div class="form-group">
                        <label>Email</label>
                        <input class="form-control{{ $errors->first('email', ' is-invalid') }}" type="email" name="email" value="{{ old('email', isset($user) ? $user->email : '') }}">
